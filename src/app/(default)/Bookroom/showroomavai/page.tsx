/////Bookroom/shoeroomavai/page.tsx
'use client'
...
const List = () => {
  ...
  const handleAddRoomAndCheckout = () => {
    // ตรวจสอบว่ามีข้อมูลครบถ้วนหรือไม่
    if (roomNo && roomType && price) {
      // สร้าง query string
      const queryString = `roomNo=${encodeURIComponent(roomNo)}&roomType=${encodeURIComponent(roomType)}&price=${encodeURIComponent(price)}`;
      
      // ส่งไปยังหน้า Bookroom/detail โดยใช้ router.push
      router.push(`/Bookroom/detail?${queryString}`);
    } else {
...
<input
  ...
  onClick={() => {
    const queryString = Object.entries({
      roomNo: room.RoomNo,
      roomType: room.RoomType?.RoomType_name,
      price: selectedRateOption === 'limited-time-offer'
